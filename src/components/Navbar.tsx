...
const navItems: NavItem[] = [
	...
	{ nameKey: "nav_soap_game", path: "/SOAPGame" },
...
const Navbar = () => {
	...
	// Only allow Home, Quiz, SOAPGame, and ContactUs for unauthenticated users
	const allowedPaths = ["/", "/Quiz", "/SOAPGame", "/ContactUs"];
	return (
						...
						{navItems.map((game) => {
							...
							return isAllowed ? (
								<Link
									key={game.path}
									to={game.path}
									className={`font-semibold transition-colors duration-200 ${location.pathname === game.path
