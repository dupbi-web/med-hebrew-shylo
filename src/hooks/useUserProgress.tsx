...
import { useCallback } from "react";
import { useToast } from "@/hooks/use-toast";
import { useUserProgressQueryV2 } from "@/hooks/queries/useUserProgressQueryV2";
import { supabase } from "@/integrations/supabase/client";
import { useAuthContext } from "@/context/AuthContext";
...
export const useUserProgress = () => {
  ...
  const getAllCategoryProgress = useCallback(
    async (categories: { id: number; totalWords: number }[]): Promise<Record<number, number>> => {
      if (!user) return {};
      ...
      try {
        ...
        categories.forEach(({ id, totalWords }) => {
          const wordIds = categoryWordMap[id] || [];
          const mastered = wordIds.filter((wId) => {
            const wp = progressByWord.get(wId);
            return wp && wp.correct > 0;
          }).length;
          result[id] = totalWords ? Math.round((mastered / totalWords) * 100) : 0;
        });
