...
const Auth = () => {
  ...
  // NEW: Separate consent checkboxes for Google OAuth
  const [googleTermsAccepted, setGoogleTermsAccepted] = useState(false);
  const [googlePrivacyAccepted, setGooglePrivacyAccepted] = useState(false);
  const [googleDataProcessingAccepted, setGoogleDataProcessingAccepted] = useState(false);
  const [googleMarketingAccepted, setGoogleMarketingAccepted] = useState(false);
  ...
  // NEW: Persist OAuth consent before redirect
  const persistOAuthConsent = () => {
    try {
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          dataProcessingAccepted: googleDataProcessingAccepted,
          marketingAccepted: googleMarketingAccepted,
          timestamp: Date.now(),
        })
      );
    } catch (e) {
  ...
  // UPDATED: Handle both email signup AND OAuth consent
  useEffect(() => {
  ...
  // UPDATED: Google sign-in with consent validation
  const handleGoogleSignIn = async () => {
    // Validate consent before OAuth redirect
    if (!googleTermsAccepted || !googlePrivacyAccepted || !googleDataProcessingAccepted) {
      setError("Please accept the required terms to continue with Google");
      toast({
        title: "Consent Required",
        description: "You must accept the Terms, Privacy Policy, and Data Processing consent to sign in with Google.",
        variant: "destructive",
      });
      return;
    }
  ...
  // NEW: Check if all required Google consents are accepted
  const googleConsentComplete = googleTermsAccepted && googlePrivacyAccepted && googleDataProcessingAccepted;
      ...
      <div className="mx-auto max-w-md py-10">
        <Card className="shadow-sm">
          ...
          <CardContent>
            ...
            {/* NEW: Google Sign-In with Consent Section */}
            <div className="space-y-4 mb-6 border rounded-lg p-4 bg-muted/30">
                ...
                <div className="space-y-2 pl-6">
                  <div className="flex items-start space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(v) => setGoogleTermsAccepted(!!v)}
                    />
                    <div className="flex-1">
                      <Label htmlFor="google-terms" className="text-xs cursor-pointer leading-tight">
                        I accept the{" "}
                  ...
                  <div className="flex items-start space-x-2">
                    <Checkbox
                      ...
                      onCheckedChange={(v) => setGooglePrivacyAccepted(!!v)}
                    />
                    <div className="flex-1">
                      <Label htmlFor="google-privacy" className="text-xs cursor-pointer leading-tight">
                        I accept the{" "}
                  ...
                  <div className="flex items-start space-x-2">
                    <Checkbox
                      id="google-dataproc"
                      checked={googleDataProcessingAccepted}
                      onCheckedChange={(v) => setGoogleDataProcessingAccepted(!!v)}
                    />
                    <Label htmlFor="google-dataproc" className="text-xs cursor-pointer leading-tight">
                  ...
                  <div className="flex items-start space-x-2">
                    <Checkbox
                      id="google-marketing"
                      checked={googleMarketingAccepted}
                      onCheckedChange={(v) => setGoogleMarketingAccepted(!!v)}
                    />
                    <Label htmlFor="google-marketing" className="text-xs cursor-pointer leading-tight text-muted-foreground">
              ...
              <Button
                ...
                disabled={loading || !googleConsentComplete}
                className="flex items-center gap-2 w-full justify-center"
              >
                <GoogleIcon className="h-5 w-5" />
                {googleConsentComplete ? "Continue with Google" : "Accept terms to enable"}
              </Button>

              {!googleConsentComplete && (
                <p className="text-xs text-muted-foreground text-center">
                  Please accept the required terms above to sign in with Google
                </p>
              )}
            </div>
